...
impl Cpu {
    ...
    fn inc_e(&mut self) {
        // INC E
        Cpu::inc_r(&mut self.registers.e, &mut self.registers.f);
        self.increment_clock(1);
    }
    fn dec_e(&mut self) {
        // DEC E
        Cpu::dec_r(&mut self.registers.e, &mut self.registers.f);
        self.increment_clock(1);
    }
    fn ld_e_d8(&mut self) {
        // LD E, d8
    ...
    fn jr_nz_s8(&mut self) {
        // JR NZ, s8
        let offset = self.fetch() as i8;
        if (self.registers.f & 0b10000000) == 0 {
            self.pc = self.pc.wrapping_add(offset as u16);
        }
        self.increment_clock(3);
    }
    ...
    fn inc_h(&mut self) {
        // INC H
        Cpu::inc_r(&mut self.registers.h, &mut self.registers.f);
        self.increment_clock(1);
    }
    fn dec_h(&mut self) {
        // DEC H
        Cpu::dec_r(&mut self.registers.h, &mut self.registers.f);
        self.increment_clock(1);
    }
    fn ld_h_d8(&mut self) {
        // LD H, d8
    ...
    fn daa(&mut self) {
        ...
        if (self.registers.f & 0b00010000) != 0 {
            // If the C flag is set
            a = a.wrapping_add(0x60);
        }
        if (self.registers.f & 0b00100000) != 0 {
